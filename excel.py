...
def vat_sum(ws, price_col, vat_sum_col):
    ws[f"{vat_sum_col}5"] = "Сума ПДВ"
    for row in ws.iter_rows(min_col=4, max_col=4, min_row=6, max_row=ws.max_row-1):
        for cell in row:
...
def get_total(ws, sum_without_excise_col, sum_without_vat_col, sum_col, vat_sum_col):
    total_sum = 0
    ...
    for row in ws.iter_rows(min_col=4, max_col=4, min_row=6, max_row=ws.max_row-1):
        for cell in row:
            total_without_vat += float(
                ws[f"{sum_without_vat_col}{cell.row}"].value)
            total_without_excise += float(
                ws[f"{sum_without_excise_col}{cell.row}"].value)
            total_sum += float(ws[f"{sum_col}{cell.row}"].value)
            total_vat_sum += float(ws[f"{vat_sum_col}{cell.row}"].value)
    ...
    ws[f"{vat_sum_col}{max_row}"] = round(total_vat_sum, 2)
...
def item_names(ws, groups_to_get_item_names, dish_code_col, dishes_group_col, item_name_col, DATABASE, config, place):
    ...
    for cell in ws[dishes_group_col]:
        if cell.value in groups_to_get_item_names:
            ...
            ws[f'{item_name_col}{cell.row}'] = db.get_item_name(
                num, DATABASE, config, place)
            i = 1
            while ws[f"C{cell.row+i}"].value == None:
                num = ws[f"{dish_code_col}{cell.row+i}"].value
                ws[f'{item_name_col}{cell.row+i}'] = db.get_item_name(
                    num, DATABASE, config, place)
                i += 1
    logger.info(
